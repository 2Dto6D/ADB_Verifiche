...
document.addEventListener('DOMContentLoaded', () => {
    const tablesToLoad = [
        { tableId: "VerificaLOINGeometrico", repo: "2Dto6D/ADB_Verifiche", filePath: "AR01Riepilogo/AR01_11_00_VerificaLOINGeometrico.csv" }
    ];

    tablesToLoad.forEach(table => {
        loadCSVToTable(table.tableId, table.repo, table.filePath);
    });
    loadCSV('CorrispondenzaIFC', '2Dto6D/ADB_Verifiche', 'FileCSVVerifiche/11_VerificaCorrispondenzaIFC_V2.csv');
    const chartsConfig = [
        {
            canvasId: 'CorrispondenzaIFCGraficoATorta',
            statsId: 'CorrispondenzaIFCstatistics',
            repo: '2Dto6D/ADB_Verifiche',
            filePath: 'FileCSVVerifiche/11_VerificaCorrispondenzaIFC_V2.csv'
        }
    ];
